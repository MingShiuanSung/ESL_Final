...
struct ADDITION : public sc_module {
	tlm_utils::simple_target_socket<ADDITION> tsock;
	...
	ADDITION(sc_module_name n) : sc_module(n), tsock("t_skt"), base_offset(0) {
		tsock.register_b_transport(this, &ADDITION::blocking_transport);
	...
	void blocking_transport(tlm::tlm_generic_payload &payload, sc_core::sc_time &delay) {
		...
		switch (cmd) {
			case tlm::TLM_READ_COMMAND:
				// cout << "READ" << endl;
				switch (addr) {
					case ADDITION_RESULT_ADDR_0:
						data_ptr[0] = c_0.read(); break;
					case ADDITION_RESULT_ADDR_1:
						data_ptr[0] = c_1.read(); break;
					case ADDITION_RESULT_ADDR_2:
						data_ptr[0] = c_2.read(); break;
					case ADDITION_RESULT_ADDR_3:
						data_ptr[0] = c_3.read(); break;
					default:
						std::cerr << "READ Error! ADDITION::blocking_transport: address 0x"
			...
			case tlm::TLM_WRITE_COMMAND:
				// cout << "WRITE" << endl;
				switch (addr) {
					case ADDITION_R_ADDR_0:
						a_0.write(data_ptr[0]); break;
					case ADDITION_R_ADDR_1:
						a_1.write(data_ptr[0]); break;
					case ADDITION_R_ADDR_2:
						a_2.write(data_ptr[0]); break;
					case ADDITION_R_ADDR_3:
						a_3.write(data_ptr[0]); break;
					case ADDITION_R_ADDR_4:
						b_0.write(data_ptr[0]); break;
					case ADDITION_R_ADDR_5:
						b_1.write(data_ptr[0]); break;
					case ADDITION_R_ADDR_6:
						b_2.write(data_ptr[0]); break;
					case ADDITION_R_ADDR_7:
						b_3.write(data_ptr[0]); break;
					default:
						std::cerr << "WRITE Error! ADDITION::blocking_transport: address 0x"
						          << std::setfill('0') << std::setw(8) << std::hex << addr << std::dec
						          << " is not valid" << std::endl;
				}
				break;
