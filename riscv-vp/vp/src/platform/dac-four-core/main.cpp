...
class BasicOptions : public Options {
   public:
	...
	addr_t mul_0_start_addr = 0x73000000;
	addr_t mul_0_size = 0x00100000;
	addr_t mul_0_end_addr = mul_0_start_addr + mul_0_size - 1;

	addr_t mul_1_start_addr = 0x73100000;
	addr_t mul_1_size = 0x00100000;
	addr_t mul_1_end_addr = mul_1_start_addr + mul_1_size - 1;

	addr_t mul_2_start_addr = 0x73200000;
	addr_t mul_2_size = 0x00100000;
	addr_t mul_2_end_addr = mul_2_start_addr + mul_2_size - 1;

	addr_t mul_3_start_addr = 0x73300000;
	addr_t mul_3_size = 0x00100000;
	addr_t mul_3_end_addr = mul_3_start_addr + mul_3_size - 1;

	addr_t mul_4_start_addr = 0x73400000;
	addr_t mul_4_size = 0x00100000;
	addr_t mul_4_end_addr = mul_4_start_addr + mul_4_size - 1;

	addr_t mul_5_start_addr = 0x73500000;
	addr_t mul_5_size = 0x00100000;
	addr_t mul_5_end_addr = mul_5_start_addr + mul_5_size - 1;

	addr_t mul_6_start_addr = 0x73600000;
	addr_t mul_6_size = 0x00100000;
	addr_t mul_6_end_addr = mul_6_start_addr + mul_6_size - 1;

	addr_t mul_7_start_addr = 0x73700000;
	addr_t mul_7_size = 0x00100000;
	addr_t mul_7_end_addr = mul_7_start_addr + mul_7_size - 1;

	addr_t add_0_start_addr = 0x73800000;
	addr_t add_0_size = 0x00100000;
	addr_t add_0_end_addr = add_0_start_addr + add_0_size - 1;

	addr_t add_1_start_addr = 0x73900000;
	addr_t add_1_size = 0x00100000;
	addr_t add_1_end_addr = add_1_start_addr + add_1_size - 1;

	addr_t add_2_start_addr = 0x73a00000;
	addr_t add_2_size = 0x00100000;
	addr_t add_2_end_addr = add_2_start_addr + add_2_size - 1;

	addr_t add_3_start_addr = 0x73b00000;
	addr_t add_3_size = 0x00100000;
	addr_t add_3_end_addr = add_3_start_addr + add_3_size - 1;
	...
	void parse(int argc, char **argv) override {
		...
		assert((mem_end_addr < clint_start_addr || mem_start_addr > add_3_end_addr) &&
...
int sc_main(int argc, char **argv) {
	...
	CombinedMemoryInterface core3_mem_if("MemoryInterface2", core3);

	SimpleMemory mem("SimpleMemory", opt.mem_size);
	SimpleTerminal term("SimpleTerminal");
	ELFLoader loader(opt.input_program.c_str());
	SimpleBus<6, 24> bus("SimpleBus");
	...
	MULTIPLY multiply_0("multiply_0");
	MULTIPLY multiply_1("multiply_1");
	MULTIPLY multiply_2("multiply_2");
	MULTIPLY multiply_3("multiply_3");
	MULTIPLY multiply_4("multiply_4");
	MULTIPLY multiply_5("multiply_5");
	MULTIPLY multiply_6("multiply_6");
	MULTIPLY multiply_7("multiply_7");
	ADDITION addition_0("addition_0");
	ADDITION addition_1("addition_1");
	ADDITION addition_2("addition_2");
	ADDITION addition_3("addition_3");
	MemoryDMI dmi = MemoryDMI::create_start_size_mapping(mem.data, opt.mem_start_addr, mem.size);
	InstrMemoryProxy instr_mem_0(dmi, core0);
	InstrMemoryProxy instr_mem_1(dmi, core1);
	InstrMemoryProxy instr_mem_2(dmi, core2);
	InstrMemoryProxy instr_mem_3(dmi, core3);
	...
	if (opt.use_instr_dmi)
		instr_mem_if_1 = &instr_mem_1;
	if (opt.use_data_dmi) {
		core1_mem_if.dmi_ranges.emplace_back(dmi);
	}
	instr_memory_if *instr_mem_if_2 = &core2_mem_if;
	data_memory_if *data_mem_if_2 = &core2_mem_if;
	if (opt.use_instr_dmi)
		instr_mem_if_2 = &instr_mem_2;
	if (opt.use_data_dmi) {
		core2_mem_if.dmi_ranges.emplace_back(dmi);
	}
	instr_memory_if *instr_mem_if_3 = &core3_mem_if;
	data_memory_if *data_mem_if_3 = &core3_mem_if;
	if (opt.use_instr_dmi)
		instr_mem_if_3 = &instr_mem_3;
	if (opt.use_data_dmi) {
	...
	core2.init(instr_mem_if_2, data_mem_if_2, &clint, entry_point, opt.mem_end_addr - 1024 * 32 * 2 - 1);
	core3.init(instr_mem_if_3, data_mem_if_3, &clint, entry_point, opt.mem_end_addr - 1024 * 32 * 3 - 1);
	...
	bus.ports[12] = new PortMapping(opt.mul_0_start_addr, opt.mul_0_end_addr);
	bus.ports[13] = new PortMapping(opt.mul_1_start_addr, opt.mul_1_end_addr);
	bus.ports[14] = new PortMapping(opt.mul_2_start_addr, opt.mul_2_end_addr);
	bus.ports[15] = new PortMapping(opt.mul_3_start_addr, opt.mul_3_end_addr);
	bus.ports[16] = new PortMapping(opt.mul_4_start_addr, opt.mul_4_end_addr);
	bus.ports[17] = new PortMapping(opt.mul_5_start_addr, opt.mul_5_end_addr);
	bus.ports[18] = new PortMapping(opt.mul_6_start_addr, opt.mul_6_end_addr);
	bus.ports[19] = new PortMapping(opt.mul_7_start_addr, opt.mul_7_end_addr);
	bus.ports[20] = new PortMapping(opt.add_0_start_addr, opt.add_0_end_addr);
	bus.ports[21] = new PortMapping(opt.add_1_start_addr, opt.add_1_end_addr);
	bus.ports[22] = new PortMapping(opt.add_2_start_addr, opt.add_2_end_addr);
	bus.ports[23] = new PortMapping(opt.add_3_start_addr, opt.add_3_end_addr);
	...
	bus.isocks[12].bind(multiply_0.tsock);
	bus.isocks[13].bind(multiply_1.tsock);
	bus.isocks[14].bind(multiply_2.tsock);
	bus.isocks[15].bind(multiply_3.tsock);
	bus.isocks[16].bind(multiply_4.tsock);
	bus.isocks[17].bind(multiply_5.tsock);
	bus.isocks[18].bind(multiply_6.tsock);
	bus.isocks[19].bind(multiply_7.tsock);
	bus.isocks[20].bind(addition_0.tsock);
	bus.isocks[21].bind(addition_1.tsock);
	bus.isocks[22].bind(addition_2.tsock);
	bus.isocks[23].bind(addition_3.tsock);
